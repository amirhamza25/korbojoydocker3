...
function Profile(props) {
  ...
  const onSubmit = async (e) => {
    ...
    const response = await axios
      .post(process.env.API_URL + "/userPanel/v1/userInfoUpdate/", info)
      .then((response) => {
                      ...
                      <form onSubmit={onSubmit} method="post" className="form-horizontal" encType="multipart/form-data">
                        <div className="form-row">
                          ...
                          <div className="form-group col-md-12">
                            ...
                            <input type="number" className="form-control" id="phone1" name="number" defaultValue={userInfo["0"].number} />
                          </div>
                          ...
                          <div className="form-group col-md-12">
                            ...
                            <input type="text" className="form-control" id="add2" defaultValue={userInfo["0"].districts} name="districts" />
                          </div>

                          <div className="form-group col-md-12">
                            <label htmlFor="add2">Blood </label>
                            <input type="text" className="form-control" id="add2" defaultValue={userInfo["0"].Blood} name="Blood" />
                          </div>
...
export async function getServerSideProps({ req }) {
  ...
  const { data } = await axios.get(process.env.API_URL + "/userPanel/v1/userInformation/" + decodedToken.userId);
  if (!data) {
    return {
      notFound: true,
    };
