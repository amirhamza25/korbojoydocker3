...
import { storeListOfVendorUserSuccess } from "../atom/listOfVendorUserOrder";
import useScript from "../commonFunction/ReloadJs";
const VendorSuccessOrderFromUser = (props) => {
  useScript("/assets/js/app.js");

  const getSuccessOrder = props.data;
  console.log(getSuccessOrder);
  const [ProductStockQty, updateSuccessOrder] = useRecoilState(storeListOfVendorUserSuccess);

  useEffect(() => {
    updateSuccessOrder(getSuccessOrder);
  }, [updateSuccessOrder]);
  ...
  //     });
  // };
  const successOrders = useRecoilValue(storeListOfVendorUserSuccess);
...
export async function getServerSideProps({ req }) {
  ...
  const { data } = await axios.get(process.env.API_URL + "/vendorPanel/v1/GetListOfOrderSuccessProductFromUser/" + decodedToken.userId);
  // console.log(data)
  if (!data) {
    return {
      notFound: true,
