...
import { storeListOfVendorUserProcess } from "../atom/listOfVendorUserOrder";
import useScript from "../commonFunction/ReloadJs";

const VendorProcessOrderFromUser = (props) => {
  useScript("/assets/js/app.js");

  const getProcessOrder = props.data;
  console.log(getProcessOrder);
  const [ProductStockQty, updateProcessOrder] = useRecoilState(storeListOfVendorUserProcess);

  useEffect(() => {
    updateProcessOrder(getProcessOrder);
  }, [updateProcessOrder]);
  ...
  //     });
  // };
  const processOrders = useRecoilValue(storeListOfVendorUserProcess);
                    ...
                    {processOrders.map((item, index) => {
                      ...
                      return (
                        <tr key={index}>
                          ...
                          <td> {streetaddress ? streetaddress.effectiveDelivery : ""}</td>
...
export async function getServerSideProps({ req }) {
  ...
  const { data } = await axios.get(process.env.API_URL + "/vendorPanel/v1/GetListOfOrderProcessProductFromUser/" + decodedToken.userId);
  // console.log(data)
  if (!data) {
    return {
      notFound: true,
