...
import { storeListOfVendorUserCancel } from "../atom/listOfVendorUserOrder";
import useScript from "../commonFunction/ReloadJs";

const VendorCancelOrderFromUser = (props) => {
  useScript("/assets/js/app.js");

  const getCancelOrder = props.data;
  console.log(getCancelOrder);
  const [ProductStockQty, updateCancelOrder] = useRecoilState(storeListOfVendorUserCancel);

  useEffect(() => {
    updateCancelOrder(getCancelOrder);
  }, [updateCancelOrder]);
  ...
  //     });
  // };
  const cancelOrders = useRecoilValue(storeListOfVendorUserCancel);
...
export async function getServerSideProps({ req }) {
  ...
  const { data } = await axios.get(process.env.API_URL + "/vendorPanel/v1/GetListOfOrderCancelProductFromUser/" + decodedToken.userId);
  // console.log(data)
  if (!data) {
    return {
      notFound: true,
