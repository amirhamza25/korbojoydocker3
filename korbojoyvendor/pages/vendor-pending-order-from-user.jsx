...
import { storeListOfVendorUserPending } from "../atom/listOfVendorUserOrder";
...
const VendorPendingOrderFromUser = (props) => {
  ...
  const [ProductStockQty, updatePendingOrder] = useRecoilState(storeListOfVendorUserPending);

  useEffect(() => {
    updatePendingOrder(getPendingOrder);
  }, [updatePendingOrder]);
  ...
  //     });
  // };
  const pendingOrders = useRecoilValue(storeListOfVendorUserPending);
...
export async function getServerSideProps({ req }) {
  ...
  const { data } = await axios.get(process.env.API_URL + "/vendorPanel/v1/GetListOfOrderPendingProductFromUser/" + decodedToken.userId);
  console.log(data);
  if (!data) {
    return {
      notFound: true,
