...
import { storeListOfVendorUserReturn } from "../atom/listOfVendorUserOrder";
import useScript from "../commonFunction/ReloadJs";

const VendorReturnOrderFromUser = (props) => {
  useScript("/assets/js/app.js");
  const getReturnOrder = props.data;
  console.log(getReturnOrder);
  const [ProductStockQty, updateReturnOrder] = useRecoilState(storeListOfVendorUserReturn);

  useEffect(() => {
    updateReturnOrder(getReturnOrder);
  }, [updateReturnOrder]);
  ...
  //     });
  // };
  const returnOrders = useRecoilValue(storeListOfVendorUserReturn);
...
export async function getServerSideProps({ req }) {
  ...
  const { data } = await axios.get(process.env.API_URL + "/vendorPanel/v1/GetListOfOrderReturnProductFromUser/" + decodedToken.userId);
  // console.log(data)
  if (!data) {
    return {
      notFound: true,
