...
const AddCategory = (props) => {
  ...
  const editCategoryList = props.editCategoryList;
  const router = useRouter();
  const id = router.query.categoryId;
  const main = editCategoryList.filter((pd) => pd.id == id);
  console.log(main);
            ...
            <div className="card-body">
              <form onSubmit={categoryHandleSubmit} className="form-horizontal" encType="multipart/form-data">
                <div className="form-group">
                  ...
                  <div className="mb-2">
                    {main.map((item, index) => (
                      <input type="text" className="form-control" name="title" placeholder={item.title} onChange={categoryHandleChange} key={index} />
                    ))}
                  </div>
                </div>
...
export async function getServerSideProps(context) {
  ...
  return {
    props: { getCatagory, getSubCatagory, editCategoryList },
  };
}
