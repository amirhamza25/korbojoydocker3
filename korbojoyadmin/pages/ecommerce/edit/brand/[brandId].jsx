...
const AddCategory = (props) => {
  ...
  const main = editBrand.filter((pd) => pd.id == id);
  console.log(main);
  // const id = console.log("sss", editCategoryList);
            ...
            <div className="card-body">
              <form onSubmit={categoryHandleSubmit} className="form-horizontal" encType="multipart/form-data">
                <div className="form-group">
                  ...
                  <div className="mb-2">
                    <input type="text" className="form-control" name="title" placeholder="name" onChange={categoryHandleChange} />
                  </div>
                </div>
            ...
            <div className="card-body">
              <form onSubmit={brandHandleSubmit} className="form-horizontal" encType="multipart/form-data">
                ...
                <div className="form-group">
                  ...
                  <div className="mb-2">
                    {main.map((item, index) => (
                      <input type="text" className="form-control" name="title" onChange={brandHandleChange} placeholder={item.title} key={index} />
                    ))}
                  </div>
                </div>
...
export async function getServerSideProps(context) {
  ...
  const { data: editCategoryList } = await axios.get(process.env.API_URL + "/GetCategoryData");
  const { data: editBrand } = await axios.get(process.env.API_URL + "/GetBrandData");
  return {
    props: { getCatagory, getSubCatagory, editCategoryList, editBrand },
  };
}
