...
const ListOfNewsFeed = (props) => {
  ...
  const getSliderList = props.data;
  const [slider, updateSliderInfo] = useState(getSliderList);
  useEffect(() => {
    updateSliderInfo(getSliderList);
  }, [getSliderList]);
  const deleteItem = async (id) => {
    const formData = { tableName: "feedback", idColumnName: "id", idValue: id };
                  ...
                  <tbody>
                    {slider.map((item, index) => (
                      <tr key={index}>
                        <td>{index + 1}</td>
                        <td>
                          <img src={process.env.API_URL + "/upload/" + item.imgs} width={150} height={150} />
                        </td>
                        <td>{item.subject}</td>
                        <td>{item.shortDiscriptions}</td>
