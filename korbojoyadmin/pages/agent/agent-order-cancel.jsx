                    ...
                    {cancelOrders.map((item, index) => {
                      return (
                        <tr key={index}>
                          ...
                          <Link href={`/agent/agentInvoice/invoiceId?invoiceId=${item.invoiceNumber}&id=${item.agentId}`} >
                            <td>
...
export async function getServerSideProps(context) {
  const { data } = await axios.get(process.env.API_URL + "/AdminAgent/v1/getPurchaseProductToCompanyCancel");
  // console.log(data)
  if (!data) {
    return {
      notFound: true,
