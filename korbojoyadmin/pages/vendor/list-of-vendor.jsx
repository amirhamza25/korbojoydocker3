...
const ListOfVendor = (props) => {
  ...
  const [vendor, updateVendorInfo] = useState(getVendorList);

  useEffect(() => {
    updateVendorInfo(getVendorList);
  }, [getVendorList]);
                        ...
                        <td>
                          <label className="radio-inline">
                            <input
                              ...
                              onClick={() => ActiveInactive(item.id, "1", "vendor")}
                            />
                            On <br />
                            <input
                              type="radio"
...
export async function getServerSideProps(context) {
  const { data } = await axios.get(process.env.API_URL + "/vendor/v1/GetVendorInfo");
  // console.log(data)
  if (!data) {
    return {
      notFound: true,
