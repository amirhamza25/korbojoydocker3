...
const CreateAgentAndSubAgent = () => {
  ...
  const agentHandleSubmit = async (e) => {
    e.preventDefault();
    var agenrType = e.target.agenrType.value;
    console.log(agenrType);
    var formData = new FormData();
    formData.append("sponsor", "Admin"); // Need to enter admin identy
    formData.append("img", file);
    formData.append("agenrType", agenrType);
    formValueAppend(formData, agent);
    const response = await axios
      .post(process.env.API_URL + "/agentPanel/av1/agentRegistration", formData)
      .then((response) => {
          ...
          <div className="card card-statistics">
            <div className="card-header">
              <div className="card-heading">
                <h4 className="card-title">Sub agent request</h4>
              </div>
            </div>
                ...
                <div className="form-row">
                  <div className="col-md-12">
                    <div className="form-group" style={{ display: "none" }}>
                      <label htmlFor="inputState">Select Sub Agent</label>
                      <div className="mb-2">
                        <input type="text" className="form-control" id="fname" name="agenrType" placeholder="Enter Sub Agent Name" value="SubAgent" />
                      </div>
                    </div>
                  </div>
                </div>
